...
const comparison4Resolved: ComparisonResolution = { ...comparison4Low, status: "resolved", winner: "win", counts: counts(3, 1) };
...
const pendingComparison = buildDiagnosisRoute({
  resolution: baseLow,
...
const resolved39 = buildDiagnosisRoute({ resolution: baseResolved, confirmationNeeds: {}, sessionSeed: "route-seed" });
...
const resolved47 = buildDiagnosisRoute({
...
const low39 = buildDiagnosisRoute({ resolution: baseLow, confirmationNeeds: {}, sessionSeed: "route-seed" });
...
const low43 = buildDiagnosisRoute({ resolution: baseLow, comparison: comparison4Low, confirmationNeeds: {}, sessionSeed: "route-seed" });
assert.equal(low43.questionIds.length, 43);
assert.equal(low43.remainingAdditionalBudget, 2);

const low45 = buildDiagnosisRoute({ resolution: baseLow, comparison: comparison4Low, confirmationNeeds: { gap: true }, sessionSeed: "route-seed" });
...
const capped = buildDiagnosisRoute({
  resolution: { kind: "resolved", primary: "win", secondary: "connect", source: "comparison" },
...
const resumed = buildDiagnosisRoute({
