...
assert.equal(detectProhibitedExpressions("静かに見えても、心の奥では誰よりも負けたくない人。").length, 0, "the approved character headline is not rejected by a broad phrase match");
assert.ok(detectProhibitedExpressions("あなたの心の奥では別のことを望んでいます。").some((item) => item.category === "inner_truth"), "a direct claim about the user's inner truth remains prohibited");
