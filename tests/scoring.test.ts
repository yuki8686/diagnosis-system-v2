...
const comparisonInput = (answers: AnswerRecord[], phase: "initial" | "additional" | "completed") => ({ expectedPair: pair, initialQuestionIds: ["VS-WC-1", "VS-WC-2"] as [string, string], additionalQuestionIds: ["VS-WC-3", "VS-WC-4"] as [string, string], answers, phase });
const result20 = aggregateComparison(questions, comparisonInput([answerFor("VS-WC-1", "win"), answerFor("VS-WC-2", "win")], "initial"));
...
const result11 = aggregateComparison(questions, comparisonInput([answerFor("VS-WC-1", "win"), answerFor("VS-WC-2", "connect")], "initial"));
...
for (const [name, types, expectedStatus, expectedWinner] of [
  ...
  const result = aggregateComparison(questions, comparisonInput(answers, "additional"));
...
assert.throws(
  () => aggregateComparison(questions, comparisonInput([answerFor("VS-WR-1", "analyze")], "initial")),
  /comparison pair/,
);

assert.throws(
  () => aggregateComparison(questions, comparisonInput([answerFor("VS-WC-1", "win"), answerFor("VS-WC-1", "connect")], "initial")),
  /Duplicate.*questionId|Duplicate comparison answer/i,
