...
`/api/feedback`はCloudflare Workerへ未移植です。旧Vercel APIコードとその依存は移植元・互換確認用として残していますが、Cloudflare Productionでは使用しない予定です。
...
- Production Workerは未デプロイであり、Stripe LiveおよびProduction Secretは未設定である。
...
- Production Firebase Project、Firestore Database、サービスアカウント、最小権限、Security Rulesの確定
- Production公開originの決定
- Stripe Liveの商品、Price、Webhook endpointと署名Secretの作成・照合
- Cloudflare Production専用Secret／Variableの登録
- Stripe Live E2Eと公開判定
