...
- ProductionではFirebase、公開origin、法務開示、Production専用report access tokenを設定済みである。Stripe Liveの5 Secretは未設定のため、購入導線は意図的に準備中のままである。
...
- FirebaseのFirestore Database、IAM最小権限、Security Rules、対象4コレクションが空であることの人間確認
- Stripe Live対象アカウントへのログイン、開始記念980円・通常1,980円の別商品／JPY／一回払いPriceの作成・照合
- `checkout.session.completed`のみを購読するProduction Webhook endpointの作成、Vercel endpointとの二重配送がないことの確認、Webhook Secretの登録
- Stripe Live Key、Price ID、販売モードのProduction Secret登録
- Live Checkoutを「支払う」直前まで確認（実決済、Live Webhook、paid reportは未実測）
