...
- Worker実装と `wrangler deploy --dry-run` は完了している。
- push、Cloudflareへの実デプロイ、Cloudflare環境変数・Secret登録は未実施である。
...
| --- | --- | --- | --- | --- | --- | --- |
| `FIREBASE_PROJECT_ID` | Variable | Test専用Firebase/Google Cloud Project | Production Project IDと分離 | Firestore REST APIのProject指定 | 購入有効化不可、Firestore処理不可 | Testデータ分離には別Projectを推奨 |
| `FIREBASE_CLIENT_EMAIL` | Variable（運用方針でSecret可） | Test Projectの専用サービスアカウント | Production用アカウントと分離 | サービスアカウントJWTの`iss` | 購入有効化不可、Firestore処理不可 | 実値を資料・スクリーンショットへ残さない |
...
- `FIREBASE_PROJECT_ID`、`FIREBASE_CLIENT_EMAIL`、`FIREBASE_PRIVATE_KEY`は、Test専用Firebase Projectと専用サービスアカウントから取得する。
...
- Firestore REST APIでOAuth tokenを取得し、`(default)` databaseの`diagnosisResults`、`diagnosisSessionIndex`、`stripeEvents`を読む・書く。使用する操作はtransaction開始、document read、transaction commit、`accessTokenHash`によるqueryである。
- コードにはTest用collection prefixやdatabase選択変数がない。TestデータをProductionから設定だけで分離するには、**別Firebase Project**を使う。Production ProjectをTest Workerへ登録しない。
- サービスアカウントには上記database/collectionの読み書きとtransactionに必要な最小権限だけを与える。具体的なIAM role名・組織ポリシーの適合は、既存Google Cloud管理者が確認する。
...
- したがって、Test用Firestoreを分離し、Test環境内でも値を不用意に変更しない。変更が必要なら、旧Test結果の閲覧互換性が失われることを許容してから行う。
...
3. Test専用Firebase Project、専用サービスアカウント、Firestore分離、最小権限を確認する。
...
## 11. VercelからCloudflareへの将来切替手順（まだ実行しない）
...
```text
Cloudflareアカウント確認：未確認
Cloudflare Test Worker名確認：未確認
Firebase Project確認：未確認
Firebase最小権限確認：未確認
Stripe Mode確認：未確認
Price金額・通貨・商品確認：未確認
